...
namespace SimpleKerningEffect.Effects
{
    ...
    [VideoEffect("文字間隔調整（縦書）", ["テキスト"], ["kerning", "文字", "テキスト", "text", "プラグイン", "plugin"], isAviUtlSupported:false, isEffectItemSupported:false)]
    public class TextSpaceVerticalEffect : VideoEffectBase
    {
        public override string Label => $"文字間隔調整（縦書） {Space.GetValue(0, 1, 30):F1}";

        [Display(GroupName = "文字間隔調整（縦書）", Name = "開始", Description = "調整する文字の開始位置")]
        ...
        [Display(GroupName = "文字間隔調整（縦書）", Name = "終了", Description = "調整する文字の終了位置")]
        [TextBoxSlider("F0", "", 1, 5)]
        [DefaultValue(1d)]
        [Range(1, 99999)]
        ...
        [Display(GroupName = "文字間隔調整（縦書）", Name = "文字間隔", Description = "文字間隔")]
